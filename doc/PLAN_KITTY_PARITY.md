...
| # | Path | Change |
|---|---|---|
| 1 | `src/ztc/services/terminals/__init__.py` (Protocol) | Add three methods + one dataclass to keep the core agnostic of any specific backend (so adding Ghostty later is a pure backend-side change, not a `save_helper.py`/`app.py` edit). (a) `reload_after_save(self) -> bool` — "Try to apply the most recent saved config to the running terminal. Returns True on success or when the backend auto-reloads natively (no action needed). Returns False when the backend cannot reload programmatically — caller falls back to the manual hint." (b) `manual_reload_hint(self) -> str \| None` — "User-facing instruction string shown when `reload_after_save()` returns False. Returns None if the backend never needs a manual hint (e.g. Alacritty: auto-reload native)." (c) `@dataclass(frozen=True) class StartupCheck` with `modal: ModalScreen[Any]` and `on_result: Callable[[Any], None]`, plus `startup_check(self, app: App) -> StartupCheck \| None` — "Hook called once at app startup. Backend may inspect its own config and return a `StartupCheck` whose modal is pushed by the core (with `on_result` as the dismiss callback). Returns None when no setup is needed." Choice rationale: separate method (do not overload `save()`'s `Path \| None` contract — both screens consume it as backup path); Protocol over `dict[str, str]` keyed by `backend.kind` (the dict approach forces editing `save_helper.py` for every new backend, defeating open-closed); Protocol over `isinstance(self.backend, KittyBackend)` branch in `on_mount` (same reason — the core would grow an `if`-ladder per backend). With these three methods, adding Ghostty later means: new file `ghostty.py` implementing the Protocol; zero edits to `save_helper.py`, `app.py`, or any screen. |
| 2 | `src/ztc/services/terminals/alacritty.py` | Add three minimal implementations: `reload_after_save(self) -> bool: return True` (Alacritty watches its own config file natively); `manual_reload_hint(self) -> str \| None: return None` (no action ever required); `startup_check(self, app) -> StartupCheck \| None: return None` (no setup modal). |
| 3 | `src/ztc/services/terminals/kitty.py` | Plus the three Protocol methods at the bottom: `manual_reload_hint(self) -> str: return "Press Ctrl+Shift+F5 in Kitty to reload."`; `startup_check(self, app)` loads `kitty.conf` (via `self.load(self.default_config_path())`), evaluates the state machine (sections below), and returns `StartupCheck(modal=KittyRemoteControlModal(), on_result=<closure>)` when the modal should appear (else `None`). The `on_result` closure receives the `KittyRemoteControlChoice | None` from the modal and handles the three branches (Enable / Skip+remember / Skip / dismiss) — appending directives via `write_remote_control_yes`/`write_listen_on_default`, or writing the `# ztc:` pref via `write_ztc_pref`, then `self.save(...)` and `app.notify(...)`. All Kitty-specific orchestration (state machine evaluation, modal-result handling, file writes, toasts) lives **inside this file** — `app.py` only does `push_screen(check.modal, check.on_result)`. (a) Helpers for `allow_remote_control`: `read_remote_control(doc) -> str \| None` returns the **effective** value (uses `_effective_entries` so values from included files are honored — same semantics as slot reads). `is_remote_control_disabled(value: str \| None) -> bool` returns True only for `"no"` and None; every other value (`yes`/`password`/`socket`/`socket-only`) is treated as a deliberate user choice. `write_remote_control_yes(doc)` appends `allow_remote_control yes` to the main config. (b) Helpers for `listen_on` (mirror the same shape): `read_listen_on(doc) -> str \| None` returns the effective value (uses `_effective_entries`). `is_listen_on_set(value: str \| None) -> bool` returns True only when `value` has real socket content; False for `None`, empty string, whitespace-only, and the literal `"none"` (Kitty's disabled-sentinel — `listen_on none` is a valid line that means "no listener"). **Case-sensitive comparison against `"none"`** (lowercase) — matches Kitty's documented sentinel exactly; non-canonical variants like `"None"`/`"NONE"` are treated as opaque socket names and the empirical reload outcome decides if they work (no defensive normalization without verifying Kitty's parser). Signature takes `value` (not `doc`) to mirror `is_remote_control_disabled`; callers use the uniform shape `read_*` then predicate. `write_listen_on_default(doc)` appends `listen_on unix:@ztc-{kitty_pid}` to the main config. `{kitty_pid}` is Kitty's canonical placeholder syntax (single braces, lowercase) — Kitty substitutes it per-instance, yielding unique sockets per Kitty process. The `ztc-` prefix namespaces the socket so it doesn't collide with sockets the user may have configured for other purposes. The resulting `KITTY_LISTEN_ON` value is opaque to ZTC and passed verbatim to `--to`. (c) `read_ztc_pref(doc, key)` and `write_ztc_pref(doc, key, value)` helpers manage a strict-prefix `# ztc:` JSON line in `kitty.conf`. **Read**: scan all lines, parse every line matching `^# ztc:` as JSON, **merge into a single dict with last-wins per key** (so multiple lines from manual edits get reconciled). Return `dict.get(key)`. **Write**: read merged dict, set `dict[key] = value`, then **delete every existing `# ztc:` line and append a single canonical line** at the end (`# ztc:` + `json.dumps(dict, sort_keys=True)`). The prefix is ZTC's namespace — dedupe is safe (no risk of clobbering unrelated user comments). Result: file always has at most one `# ztc:` line after a write. (d) `reload_after_save(self) -> bool`: dual-attempt loop. Reads `os.environ.get("KITTY_LISTEN_ON")` once. For each binary in `(kitty, kitten)`: builds `[binary, @]`, appends `--to <env_value>` if env var set, appends `load-config`, runs `subprocess.run(cmd, timeout=2, capture_output=True)`; returns True on first `returncode == 0`. Catches `Exception` broadly (FileNotFoundError, TimeoutExpired, etc.) — reload is opportunistic, no error must ever propagate to make a successful save look failed. Returns False if both attempts fail. The `--to` path is the working solution for inside-Zellij deployment (smoke test 7 phase 5: OK); the plain form is preserved for users outside Zellij who have only `allow_remote_control yes` without `listen_on`. |
...
| 6 | `src/ztc/app.py` (`TermConfigApp.on_mount` or post-mount hook) | After backend detection: `if self.backend is not None: check = self.backend.startup_check(self); if check is not None: self.push_screen(check.modal, check.on_result)`. **Zero `isinstance` branches, zero state-machine logic, zero modal-specific knowledge** — the core only orchestrates the push. All Kitty-specific decisions (when to show the modal, what the modal looks like, what to do with each result) live in `kitty.py`. Adding Ghostty later means implementing `startup_check` in `ghostty.py`; this hook keeps working untouched. |
...
| 12 | `tests/test_app_menu_gating.py` (or new file) | Two layers of tests, mirroring the responsibility split: (a) **In `tests/test_terminal_kitty.py`**: `KittyBackend.startup_check(mock_app)` returns the right `StartupCheck` (or `None`) for each row of the state machine table; the `on_result` callback, invoked with each `KittyRemoteControlChoice` value, writes the right directives / `# ztc:` pref and notifies the right toast (use a stub app that records `notify` and `push_screen` calls). (b) **In `test_app_menu_gating.py`**: `app.py` invokes `backend.startup_check(self)` exactly once during startup and pushes the returned modal — using a fake backend whose `startup_check` returns a sentinel `StartupCheck`, no Kitty knowledge in the test. Also: when backend is `None` (SSH/unsupported), `startup_check` is not called. This split mirrors the production split (core agnostic, backend concrete). |
...
> Note on Protocol surface: this plan adds **three** methods to
> `TerminalBackend`: `reload_after_save()`, `manual_reload_hint()`,
> and `startup_check(app)`. The earlier draft of this plan kept
> `manual_reload_hint` as a `dict[str, str]` inside `save_helper.py`
> and the startup logic as an `isinstance(self.backend, KittyBackend)`
> branch in `app.py` — both rejected once the next-terminal target
> (Ghostty) was made explicit. Either shortcut would force editing
> `save_helper.py` and `app.py` for every new backend, which defeats
> the whole point of the Protocol. The three Protocol methods make
> Ghostty (and any future backend) a pure backend-side addition:
> a new file implementing the Protocol, zero edits in the core.
