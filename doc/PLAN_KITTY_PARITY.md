...
| # | Path | Change |
|---|---|---|
| 1 | `src/ztc/services/terminals/__init__.py` (Protocol) | Add `reload_after_save(self) -> bool` to `TerminalBackend`. Doc: "Try to apply the most recent saved config to the running terminal. Returns True on success or when the backend auto-reloads natively (no action needed). Returns False when the backend cannot reload programmatically — caller is expected to fall back to user-facing instructions." Choice rationale: separate method, do not change `save()`'s `Path | None` contract (that one returns the backup path; both screens use it as such). |
| 2 | `src/ztc/services/terminals/alacritty.py` | Add `def reload_after_save(self) -> bool: return True`. Alacritty watches its own config file — no programmatic action needed; treat as success. |
...
| 5 | `src/ztc/services/save_helper.py` (new) | (a) `save_with_reload(backend, doc, path) -> SaveResult` encapsulates the save+reload sequence: invoke `backend.save(doc, path)` (captures backup path), invoke `backend.reload_after_save()` (captures bool), look up the manual-reload hint internally via a per-backend mapping. Returns a dataclass with **only data**: `backup_path: Path \| None`, `reload_ok: bool`, `manual_reload_hint: str \| None` (set when `reload_ok=False`). Errors from `backend.save` propagate. (b) `compose_save_toast(file_name: str, result: SaveResult) -> str` composes the toast format used by the two save screens: "Saved: <file_name>" + optional "(backup: <name>)" + optional "\\n<hint>" when reload failed. **Not used by theme_sync** — that caller has its own message format ("Kitty updated: N slot(s)") and composes its own toast using the same `result` data. |
| 6 | `src/ztc/app.py` (`TermConfigApp.on_mount` or post-mount hook) | After backend detection, if `self.backend` is `KittyBackend`, load the config and evaluate the state machine: skip if `allow_remote_control` is `password`, skip if both `allow_remote_control` is permissive AND `listen_on` is set, skip if dismissal comment present. Otherwise push `KittyRemoteControlModal`. On "Enable": determine which directive(s) are missing (one or both per state machine table), append only those (using `write_remote_control_yes` and/or `write_listen_on_default`), save the doc, show toast naming the count of lines added and prompting Kitty restart. On "Skip" + checkbox: write dismissal comment via `write_ztc_pref` then save. On "Skip" without checkbox: nothing. |
...
| 12 | `tests/test_app_menu_gating.py` (or new file) | Tests for the startup hook: backend Kitty + each row of the state machine table → expected modal pushed or not. Uses monkey-patching of `push_screen` (same pattern as Stage A integration tests). Also tests the modal Enable callback: each combination of missing directives → writes only what's missing and toast count matches. |
...
> Note on Protocol surface: `manual_reload_hint()` is intentionally
> **not** added to the Protocol. The hint is internal to
> `save_helper.py` — a `dict[str, str]` keyed by `backend.kind`
> maps Kitty to its `"Press Ctrl+Shift+F5 in Kitty to reload."`
> string; Alacritty has no entry (auto-reload, no hint needed).
> Keeps the Protocol surface minimal.
...
- **DRY via full `save_with_reload()` helper, not just toast
  ...
  message. Centralizing only the message (initial pick) leaves
  the screens orchestrating the sequence and querying both
  `reload_after_save()` and `manual_reload_hint()` directly. With
  the full helper, screens reduce to "call helper, show toast,
  handle error". Three concrete wins: (a) no `manual_reload_hint()`
  in the Protocol — the hint is private to the helper, mapped by
  `backend.kind`; (b) screens never reference `reload_after_save()`
  either; (c) test surface drops — `test_save_helper.py` covers
  the sequence once, screen tests just verify "consume result".
